...
<header class="relative w-full min-h-[400px] overflow-hidden" aria-hidden="true">
  <img src="<?= htmlspecialchars($hero_img) ?>" alt=""
    class="absolute inset-0 w-full h-full min-h-[400px] object-cover object-center" width="1200" height="700">
  <div class="absolute inset-0 bg-gradient-to-t from-gray-900/60 to-transparent pointer-events-none"></div>
</header>
<div class="max-w-2xl mx-auto px-4 py-10">
  <section class="mb-8 text-center">
    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-4"><?= htmlspecialchars($event_title) ?></h1>
    <p class="flex items-center justify-center gap-2 text-lg text-gray-600 mb-2">
      ...
      <a href="https://www.google.com/maps/search/?api=1&amp;query=658+Gibraltar+Court,+Milpitas,+CA+95035"
        target="_blank" rel="noopener noreferrer" class="text-indigo-600 hover:text-indigo-800 hover:underline">658
        Gibraltar Court, Milpitas, CA 95035</a>
    </p>
    <p class="flex items-center justify-center gap-2 text-lg text-gray-600 mb-6">
      <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  <?php if (isset($_GET['cancelled'])): ?>
    <div class="card border-amber-200 bg-amber-50 mb-6 text-amber-800">Payment was cancelled. You can complete the form
      again when ready.</div>
  <?php endif; ?>
    ...
    <form method="post" action="" id="registration-form" data-initial-step="<?= (int) $initial_step ?>">
      ...
      <div class="mb-8 <?= $show_early_bird ? 'grid grid-cols-1 sm:grid-cols-2 gap-4' : '' ?>">
        <?php if ($show_early_bird): ?>
          <div class="rounded-xl  bg-amber-100 px-5 py-4 shadow-sm">
            ...
            <p class="mt-1 text-2xl font-bold text-amber-900"><?= format_money($early_bird_price_cents / 100.0) ?> <span
                class="text-lg font-semibold text-amber-900/90">per child</span></p>
          ...
          <div class="rounded-xl -white px-5 py-4 shadow-sm">
            ...
            <p class="mt-1 text-2xl font-bold text-gray-900"><?= format_money($regular_price_cents / 100.0) ?> <span
                class="text-lg font-semibold text-gray-600">per child</span></p>
          </div>
        <?php else: ?>
      ...
      <div id="step-1" class="registration-step step-panel" aria-label="Step 1 – Parent &amp; emergency contact">
        <div class="flex items-center gap-2 mb-6">
          <span
            class="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-medium text-white">1</span>
        ...
        <div class="card mb-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" id="parent_first_name" name="parent_first_name" required maxlength="100"
            ...
            <div>
              ...
              <input type="text" id="parent_last_name" name="parent_last_name" required maxlength="100"
          ...
          <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label for="alternative_pickup_name" class="block text-sm font-medium text-gray-700 mb-1">Alternative pick
                up name <span class="text-gray-500 font-normal"></span></label>
            ...
            <div>
              <label for="alternative_pickup_phone" class="block text-sm font-medium text-gray-700 mb-1">Alternative pick
                up phone <span class="text-gray-500 font-normal"></span></label>
      ...
      <div id="step-2" class="registration-step step-panel hidden" aria-label="Step 2 – Add kids">
        <div class="flex items-center gap-2 mb-6">
          <span
            class="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-medium text-white">2</span>
          ...
          <div id="kids-container" class="space-y-4">
            <?php foreach ($kids_for_form as $idx => $kid): ?>
              <div class="kid-block border border-gray-200 rounded-lg p-4 bg-gray-50/50" data-index="<?= $idx ?>">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label for="kids-<?= $idx ?>-first_name" class="block text-sm font-medium text-gray-700 mb-1">First name
                      *</label>
                    <input type="text" id="kids-<?= $idx ?>-first_name" name="kids[<?= $idx ?>][first_name]" required
                      maxlength="100" value="<?= htmlspecialchars($kid['first_name']) ?>" class="input-field">
                  </div>
                  <div>
                    <label for="kids-<?= $idx ?>-last_name" class="block text-sm font-medium text-gray-700 mb-1">Last name
                      *</label>
                    <input type="text" id="kids-<?= $idx ?>-last_name" name="kids[<?= $idx ?>][last_name]" required
      ...
      <div id="step-3" class="registration-step step-panel hidden" aria-label="Step 3 – Consent &amp; payment">
        <div class="flex items-center gap-2 mb-6">
          <span
            class="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-sm font-medium text-white">3</span>
        ...
        <div class="card mb-6">
          ...
          <input type="text" id="digital_signature" name="digital_signature" required maxlength="200"
