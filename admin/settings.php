...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $price_per_kid_cents = (int) ($_POST['price_per_kid_cents'] ?? 0);
    $currency = trim($_POST['currency'] ?? 'usd');
    $early_bird_start = trim($_POST['early_bird_start_date'] ?? '');
    $early_bird_end = trim($_POST['early_bird_end_date'] ?? '');
    $early_bird_discount_percent = (float) ($_POST['early_bird_discount_percent'] ?? 0);
    $multi_kid_discount_percent = (float) ($_POST['multi_kid_discount_percent'] ?? 0);
    $multi_kid_min_count = (int) ($_POST['multi_kid_min_count'] ?? 2);
    $max_kids_per_registration = (int) ($_POST['max_kids_per_registration'] ?? 10);
    $consent_form_url = trim($_POST['consent_form_url'] ?? '#');
    $registration_open = isset($_POST['registration_open']) ? '1' : '0';

    if ($price_per_kid_cents < 0) $errors[] = 'Price per kid cannot be negative.';
    ...
    if (empty($errors)) {
        $keys = [
            ...
            'early_bird_discount_percent', 'multi_kid_discount_percent', 'multi_kid_min_count',
            'max_kids_per_registration', 'consent_form_url', 'registration_open'
        ];
        $stmt = $pdo->prepare("INSERT INTO settings (`key`, `value`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)");
        $stmt->execute(['price_per_kid_cents', (string) $price_per_kid_cents]);
        ...
        $stmt->execute(['early_bird_discount_percent', (string) $early_bird_discount_percent]);
        $stmt->execute(['multi_kid_discount_percent', (string) $multi_kid_discount_percent]);
        ...
        $stmt->execute(['consent_form_url', $consent_form_url]);
...
$price_per_kid_cents = (int) ($settings['price_per_kid_cents'] ?? 5000);
$currency = $settings['currency'] ?? 'usd';
$early_bird_start = $settings['early_bird_start_date'] ?? '';
$early_bird_end = $settings['early_bird_end_date'] ?? '';
$early_bird_discount_percent = $settings['early_bird_discount_percent'] ?? '0';
$multi_kid_discount_percent = $settings['multi_kid_discount_percent'] ?? '0';
$multi_kid_min_count = $settings['multi_kid_min_count'] ?? '2';
$max_kids_per_registration = $settings['max_kids_per_registration'] ?? '10';
$consent_form_url = $settings['consent_form_url'] ?? '#';
...
<div class="max-w-3xl mx-auto px-4 py-8">
  ...
  <form method="post" action="" class="space-y-6">
    <div class="card">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label for="price_per_kid_cents" class="block text-sm font-medium text-gray-700 mb-1">Price per kid (cents)</label>
          <input type="number" id="price_per_kid_cents" name="price_per_kid_cents" min="0" value="<?= (int) $price_per_kid_cents ?>" class="input-field">
          <p class="text-xs text-gray-500 mt-1">e.g. 5000 = $50.00</p>
        </div>
        <div>
    ...
    <div class="card">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          <label for="early_bird_discount_percent" class="block text-sm font-medium text-gray-700 mb-1">Discount %</label>
          <input type="number" id="early_bird_discount_percent" name="early_bird_discount_percent" min="0" max="100" step="0.01" value="<?= htmlspecialchars($early_bird_discount_percent) ?>" class="input-field">
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label for="multi_kid_discount_percent" class="block text-sm font-medium text-gray-700 mb-1">Discount %</label>
          <input type="number" id="multi_kid_discount_percent" name="multi_kid_discount_percent" min="0" max="100" step="0.01" value="<?= htmlspecialchars($multi_kid_discount_percent) ?>" class="input-field">
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="space-y-4">
        ...
        <div>
          <label for="consent_form_url" class="block text-sm font-medium text-gray-700 mb-1">Consent form URL</label>
          <input type="url" id="consent_form_url" name="consent_form_url" value="<?= htmlspecialchars($consent_form_url) ?>" class="input-field">
        </div>
  ...
  <div class="mt-8 p-4 bg-gray-100 rounded-lg text-sm text-gray-600">
    <strong>Preview:</strong> 1 kid = <?= format_money(compute_total_cents($pdo, 1)) ?>,
    2 kids = <?= format_money(compute_total_cents($pdo, 2)) ?>,
    3 kids = <?= format_money(compute_total_cents($pdo, 3)) ?>.
  </div>
</div>
</body>
</html>
